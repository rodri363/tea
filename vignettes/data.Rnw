...
\title{TEA Data Handling (Version \Sexpr{foo$Version})}
...
.quote = What character (if any) is used to surround character strings in the file
.na = What sequences of characters are used to denote missing values in the file
for delimited files, extra options are:
.delim = What is the character used to delimit text (for a delimited file)
.header = Does the text file contain a list of variable names in the first row
for fixed width files, extra options are:
.widths = What are the lengths of each field (in a fixed-width file)
.names = What are the names of each variable
...
operations.  These are variables like incomes, age, lot measurements, number of a given item, etc.
...
The default in TEA is that all variables are text unless otherwise specified.
...
%this will be a vignette example in the finished doc
> names(DF)
"A" "B"
...
of inconsistencies from occuring!  Thus we need a mechanism for capturing actions on
a view and taking appropriate action.  In SQL, this mechanism is known as a ``trigger''.
