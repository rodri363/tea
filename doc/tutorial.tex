...
%  DV: By the configuration system we mean the spec file, correct?
%      If so we should specify that so as not to confuse the user.
...
%  DV: Again, some of this could be made more accessible to the reader by 
%  first explaining how checks, input, etc get used. Although much of the 
%  syntax in the spec is SQLite stuff that the reader is told to learn it 
%  would give the code context to explain what's happening.
\begin{lstlisting}[language=]
...
view table.
%  DV: Where do you specify what view table to write to? Is it in the input section of 
%  the spec file as the key "output table"? If so, does the outpu of the impute 
%  functions get written to the view table as well or directly to the database?



%  DV: By declaration list (line 550), do we mean the declaration of the variable type 
%  in the database that we specified in the first line of the spec file or the declaration 
%  type that we specify in field?
