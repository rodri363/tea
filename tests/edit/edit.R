...
read_spec("spec") #read in the spec; this primes the consistency checking system
doInput() #input the data as described in the input{} section of the spec

#read the data into an R data frame
DF <- dbGetQuery(pepenv$con,"select * from data")
...
va <- as.integer(CheckDF(DF,pepenv$con))
